...
def load_imagenet_pretrain(model, checkpoint_file):
    ...
    #### only convert RGB channel weights to conv1, the rest are zero
...
def get_unknown_tensor_from_pred(pred, rand_width=30, train_mode=True):
    ...
    #### weight uncertain_area is 1
    return weight

def get_unknown_tensor_from_pred_T(pred, rand_width=30, train_mode=True):
    ### pred: N, 1 ,H, W
    N, C, H, W = pred.shape
